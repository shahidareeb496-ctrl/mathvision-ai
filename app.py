...
def clean_ai_response(text):
    if not text:
        return ""
    # Remove obvious reasoning artifacts
    artifacts = [
        r"<\|channel\|>analysis",
        r"<\|start\|>",
        r"<\|end\|>",
...
def ai_generate(prompt, temperature=0.7, image_bytes=None, filename=None):
    # Appending direct instruction to all text-only prompts
    if not image_bytes:
        prompt += "\nRespond with ONLY the final answer. Do not include any reasoning, thinking, or process explanation."
...
def api_chat():
    ...
    # Build system prompt in specific order: Identity -> Purpose -> Biology -> Topic
    system_prompt = f"You are SynapticAI, an interactive Biology learning assistant. "
    if name:
        system_prompt += f"The student's name is {name}. "

    system_prompt += "Your purpose is helping students understand biology through interactive 3D models. "
    system_prompt += "Explain using simple educational language. "
    system_prompt += "Do not answer as a study planner or schedule assistant. "

    if schedule:
        system_prompt += f"For context, the student's study plan is: {json.dumps(schedule)}. "
        system_prompt += f"Completed tasks: {json.dumps(completed)}. "

    if topic:
        system_prompt += f"\n\nCRITICAL CONTEXT: The current discussion topic is: {topic}. "
        system_prompt += f"Treat {topic} as the current biological structure or concept. "
        system_prompt += f"All references such as: 'it', 'its', 'this', 'this organelle', 'this structure' "
        system_prompt += f"must always refer to {topic} unless the user explicitly changes the subject. "
        system_prompt += f"Your conversation should be restricted to this topic only. Do not drift into unrelated biology. "
        system_prompt += f"If asked about subjects unrelated to {topic}, politely redirect the student back to it. "

    # CLEANING HISTORY: Remove any 'system' messages
    original_history_size = len(history)
    clean_history = [m for m in history if m.get("role") != "system"]
    system_messages_removed = original_history_size - len(clean_history)

    # Check for "Study Planner" contamination in assistant messages
    study_planner_keywords = ["Study Planner", "Study Coach", "schedule", "academic doubts", "priorities", "motivation"]
    study_planner_contamination = [
        m for m in clean_history
        if m.get("role") == "assistant" and any(k.lower() in m.get("content", "").lower() for k in study_planner_keywords)
    ]

    print(f"DEBUG: History Cleaning - Original Size: {original_history_size}, Removed: {system_messages_removed}")
    if study_planner_contamination:
        print(f"DEBUG: History Contamination - Found {len(study_planner_contamination)} Study Planner messages in history.")

    # USER MESSAGE REWRITING (Payload only)
    # Resolve pronouns and prepend topic context if it's the first message of a topic
    topic_context_established = False
    if topic:
        topic_lower = topic.lower()
        topic_context_established = any(topic_lower in m.get("content", "").lower() for m in clean_history if m.get("role") == "user")

    if topic and not topic_context_established:
        # Resolve pronouns/ambiguous references in the message for better model understanding
        resolved_message = message
        # Pattern: "its <word>" -> "the <word> of the <topic>"
        resolved_message = re.sub(r"\bits\s+(\w+)\b", rf"the \1 of the {topic}", resolved_message, flags=re.IGNORECASE)
        # Patterns for standalone ambiguous terms
        replacements = {
            r"\bits\b": f"the {topic}'s",
            r"\bit\b": topic,
            r"\bthis organelle\b": topic,
            r"\bthis structure\b": topic,
            r"\bthis\b": topic
        }
        for pattern, replacement in replacements.items():
             resolved_message = re.sub(pattern, replacement, resolved_message, flags=re.IGNORECASE)

        payload_message = (
            f"Current Biology Topic: {topic}.\n\n"
            f"The following question refers ONLY to this biology topic.\n\n"
            f"Question:\n{resolved_message}"
        )
    else:
    ...
    try:
        ...
        if image_bytes:
            encoded = base64.b64encode(image_bytes).decode("ascii")
            content = [{"type": "text", "text": payload_message}] if message else []
            content.append({
                "type": "image_url",
            ...
            # Final trace before vision completion call
            model_name = "qwen/qwen3.6-27b"
            print(f"DEBUG (Vision): Model: {model_name}, Payload: {json.dumps(messages, indent=2)}")
            ...
            # Final trace before completion call
            model_name = "openai/gpt-oss-120b"
            print(f"DEBUG: Model: {model_name}, Payload: {json.dumps(messages, indent=2)}")
        ...
        print(f"DEBUG: Raw Groq Response Object: {response}")
        raw_text = response.choices[0].message.content
        print(f"DEBUG: Raw Completion Text: {raw_text}")

        answer = clean_ai_response(raw_text)
        print(f"DEBUG: Final API Response: {answer}")
        return jsonify({"answer": answer})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def api_quiz_evaluate():
    data = request.json
    subject = data.get("subject", "General Math/Science")
    chapter = data.get("chapter", "")
    results = data.get("results", [])

    # If results is empty but someone sent a single question (like in the manual test)
    if not results and data.get("question"):
        results = [{
            "question": data.get("question"),
            "user_answer": data.get("student_answer", data.get("user_answer", "")),
            "correct_answer": data.get("correct_answer", "")
        }]

    if not results:
        return jsonify({"error": "No quiz results to evaluate"}), 400

    # Harden the search query to target 10th grade PCTB specifically
    query = f"Punjab Board PCTB 10th grade 2026 syllabus key concepts and common mistakes for {subject} {chapter}. Focus on secondary school education."
    search_data = tavily_search(query)
    search_context = ""
    if search_data:
...
If the provided context is about medicine, pharmacy, or other unrelated fields, IGNORE IT and use your internal knowledge of 10th grade {subject} for the Punjab Board.
...
{{
  ...
  "study_suggestion": "..."
    ...
    try:
        ...
        # Ensure evaluations array is not empty if there were questions
        if results and not evaluation.get("evaluations"):
             evaluation["evaluations"] = [{"question": r["question"], "category": "Concept Misunderstanding", "explanation": "General error in solving."} for r in results if r["user_answer"] != r["correct_answer"]]
