...
def api_chat():
    ...
    # Build system prompt: Identity -> Vision Suppression -> Purpose -> Topic
    system_prompt = "You are SynapticAI, an interactive Biology learning assistant. "
    system_prompt += "IMPORTANT: You are in TEXT-ONLY mode. You cannot see any images or the user's screen. "
    system_prompt += "Rely entirely on the text context provided. "

    if topic:
        system_prompt += f"The current biological discussion topic is: {topic}. "
        system_prompt += f"All ambiguous references such as: 'it', 'its', 'this', 'this organelle', 'this structure' "
    ...
    system_prompt += "Your purpose is helping students understand biology. Explain using simple educational language. "
    system_prompt += "Do not answer as a study planner or schedule assistant. "

    if topic:
        system_prompt += f"\n\nCRITICAL CONTEXT: You must stay restricted to the topic of '{topic}'. "
        system_prompt += "Do not drift into unrelated biology. "
    ...
    # Explicitly resolve pronouns/ambiguities to force model attention on the topic
    if topic:
        resolved = message
        # 1. Handle "its <word>" -> "the <word> of the <topic>"
        resolved = re.sub(r"\bits\s+(\w+)\b", rf"the \1 of the {topic}", resolved, flags=re.IGNORECASE)
        # 2. Handle "it" or "this" as objects
        resolved = re.sub(r"\b(it|this|this structure|this organelle)\b", topic, resolved, flags=re.IGNORECASE)

        payload_message = (
            f"The following question refers ONLY to the biology topic: {topic.upper()}.\n\n"
            f"Question:\n{resolved}"
        )
    else:
