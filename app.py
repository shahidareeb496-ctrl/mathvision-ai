...
def api_chat():
    ...
    if topic:
        system_prompt += f"You are currently helping them with the biological topic: '{topic}'. "
        system_prompt += "Your conversation should be restricted to this topic only. "
    ...
    try:
        ...
        if image_bytes:
            ...
            if message:
                messages.append({"role": "user", "content": message + "\nRespond with ONLY the final answer. No reasoning."})
            response = client.chat.completions.create(
                model="openai/gpt-oss-120b",
                messages=messages,
                temperature=0.7,
